...
func assertValidNetmap(t *testing.T, client TailscaleClient) {
	...
	if netmap.SelfNode.Online().Valid(){
	...
	for _, peer := range netmap.Peers {
		assert.NotEqualf(t, "127.3.3.40:0", peer.HomeDERP(), "peer (%s) has no home DERP in %q's netmap, got: %s", peer.ComputedName(), client.Hostname(), peer.HomeDERP())
