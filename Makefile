...
# Install tools required by `go generate` (buf, grpc-gateway, protoc-gen-go).
# Run this once in a fresh environment if not using `nix develop`.
.PHONY: install_grpc_tools
install_grpc_tools:
	go install \
