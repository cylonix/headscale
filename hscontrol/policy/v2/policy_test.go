...
// TestSetTailnetPolicy_FallbackAndOverride exercises the per-tailnet
// policy path used by BuildPeerMapForTailnet. It verifies that:
//
//  1. Without any per-tailnet policy installed, BuildPeerMapForTailnet
//     uses the global policy — two nodes that the global policy allows
//     to talk see each other as peers.
//  2. After SetTailnetPolicy installs a restrictive per-tailnet policy
//     for tailnet "tnA", BuildPeerMapForTailnet("tnA", ...) returns no
//     peers — proving the per-tailnet matchers are the ones consulted.
//  3. ClearTailnetPolicy("tnA") restores the fallback to global, and
//     BuildPeerMapForTailnet("tnA", ...) again returns the global-policy
//     peer relationships.
//
// This is the regression test for the cylonix per-tailnet policy
// resolution refinement.
func TestSetTailnetPolicy_FallbackAndOverride(t *testing.T) {
	users := types.Users{
		{Model: gorm.Model{ID: 1}, Name: "user1", Email: "user1@headscale.net"},
	}

	n1 := node("node1", "100.64.0.1", "fd7a:115c:a1e0::1", users[0], nil)
	n1.ID = 1
	n1.NetworkDomain = "tnA"
	n2 := node("node2", "100.64.0.2", "fd7a:115c:a1e0::2", users[0], nil)
	n2.ID = 2
	n2.NetworkDomain = "tnA"

	nodes := types.Nodes{n1, n2}

	// Global policy: allow user1 to talk to itself (so n1 <-> n2 should
	// be peers under the global policy).
	globalPol := `{
		"acls": [
			{
				"action": "accept",
				"src": ["user1@headscale.net"],
				"dst": ["user1@headscale.net:*"]
			}
		]
	}`

	pm, err := NewPolicyManager([]byte(globalPol), users, nodes.ViewSlice())
	require.NoError(t, err)

	ownNodes := views.SliceOf([]types.NodeView{n1.View(), n2.View()})
	emptyShare := views.SliceOf([]types.NodeView(nil))

	// Step 1: no per-tailnet policy → falls back to global → n1<->n2 are peers.
	peerMap := pm.BuildPeerMapForTailnet("tnA", ownNodes, emptyShare, emptyShare)
	require.NotNil(t, peerMap)
	assert.Len(t, peerMap[n1.ID], 1, "global policy should give n1 one peer (n2)")
	assert.Len(t, peerMap[n2.ID], 1, "global policy should give n2 one peer (n1)")

	// Step 2: install a per-tailnet policy that has NO acls → no peer
	// relationships in this tailnet.
	emptyAclsPol := `{"acls": []}`
	changed, err := pm.SetTailnetPolicy("tnA", []byte(emptyAclsPol))
	require.NoError(t, err)
	assert.True(t, changed, "first install of per-tailnet policy should report changed=true")

	peerMap = pm.BuildPeerMapForTailnet("tnA", ownNodes, emptyShare, emptyShare)
	assert.Empty(t, peerMap[n1.ID], "per-tailnet policy with no acls should yield no peers for n1")
	assert.Empty(t, peerMap[n2.ID], "per-tailnet policy with no acls should yield no peers for n2")

	// Idempotent re-install: same bytes should report changed=false.
	changed, err = pm.SetTailnetPolicy("tnA", []byte(emptyAclsPol))
	require.NoError(t, err)
	assert.False(t, changed, "re-installing the same per-tailnet policy should report changed=false")

	// Step 3: clear → fallback to global → peers reappear.
	cleared := pm.ClearTailnetPolicy("tnA")
	assert.True(t, cleared, "clearing an installed per-tailnet policy should return true")

	peerMap = pm.BuildPeerMapForTailnet("tnA", ownNodes, emptyShare, emptyShare)
	assert.Len(t, peerMap[n1.ID], 1, "after clear, fallback to global policy should restore peer n2 for n1")
	assert.Len(t, peerMap[n2.ID], 1, "after clear, fallback to global policy should restore peer n1 for n2")

	// Clearing again is a no-op.
	cleared = pm.ClearTailnetPolicy("tnA")
	assert.False(t, cleared, "clearing an already-cleared tailnet should return false")

	// SetTailnetPolicy with empty bytes should also clear (no-op since
	// already cleared).
	changed, err = pm.SetTailnetPolicy("tnA", nil)
	require.NoError(t, err)
	assert.False(t, changed, "SetTailnetPolicy(nil) on a cleared tailnet should report changed=false")

	// Empty tailnet identifier is a programming error.
	_, err = pm.SetTailnetPolicy("", []byte(emptyAclsPol))
	require.Error(t, err, "SetTailnetPolicy with empty tailnet should error")
}
