...
// [Headscale.waitForFollowup]. That poll is authenticated only by the
// registration ID embedded in the followup URL, so without a machine-key
// check anyone who learns an ID gets the registering user's User/Login back
// in the [tailcfg.RegisterResponse].
//
...
func TestWaitForFollowupMachineKeyMismatch(t *testing.T) {
	...
	// Park a completed registration in the registration cache, as a node that
	// is already polling for its verdict would see it.
