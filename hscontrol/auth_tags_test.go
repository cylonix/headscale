...
func TestTaggedNodeRestartPreservesNilExpiry(t *testing.T) {
	...
	require.Equal(t, types.TaggedDevices.View().TailscaleUser(app.cfg), restartResp.User,
...
func TestUntaggedNodeRestartPreservesNilExpiry(t *testing.T) {
	// The fork's createTestApp configures no default node expiry, matching
	// upstream's createTestAppWithNodeExpiry(t, 0).
	app := createTestApp(t)
