...
import (
	...
	"golang.org/x/oauth2"
	"tailscale.com/types/key" // __CYLONIX_ADD__ used by registerNodeForOIDCCallback
	"zgo.at/zcache/v2"
)
...
// registerNodeForOIDCCallback finalises a node registration after the OIDC
// followup URL has been redeemed. It looks up the in-flight registration
// cache entry by MachineKey (cylonix's auth flow only has the MachineKey at
// this point, not the v0.28 RegistrationID), then delegates to
// state.HandleNodeFromAuthPath for the actual DB insert + change broadcast.
// Returns nil with no error if the cache entry is missing — that path is
// expected when the upstream AuthProviderOIDC flow has already finalised the
// registration via its own RegistrationID handle.
func (h *Headscale) registerNodeForOIDCCallback(
	...
	if user == nil || machineKey == nil {
		return fmt.Errorf("registerNodeForOIDCCallback: user and machineKey are required")
	}
	regID, ok := h.state.FindRegistrationIDByMachineKey(*machineKey)
	if !ok {
		// No in-flight registration; assume the upstream AuthProviderOIDC
		// flow already finalised it via state.HandleNodeFromAuthPath.
		log.Debug().
			Str("machine_key", machineKey.ShortString()).
			Str("user", user.Name).
			Msg("registerNodeForOIDCCallback: no pending registration in cache")
		return nil
	}
	exp := expiry
	nodeView, c, err := h.state.HandleNodeFromAuthPath(regID, types.UserID(user.ID), &exp, util.RegisterMethodOIDC)
	if err != nil {
		return fmt.Errorf("registerNodeForOIDCCallback: HandleNodeFromAuthPath: %w", err)
	}
	// __BEGIN_CYLONIX_ADD__
	// Cylonix NodeHandler.PostAdd populates the cylonix-side wg_info / wg_node
	// rows that drive the manager UI's machines page. The upstream
	// state.HandleNodeFromAuthPath only writes the headscale `nodes` table —
	// without this call the device registers in headscale but is invisible
	// to cylonix-manager. Pre-v0.28 the gRPC node-create path was the
	// dispatching site; v0.28's auth-path flow needs an explicit hook here.
	if h.cfg.NodeHandler != nil && nodeView.Valid() {
		node := nodeView.AsStruct()
		if err := h.cfg.NodeHandler.PostAdd(node); err != nil {
			log.Error().
				Err(err).
				Uint64("node-id", node.ID.Uint64()).
				Str("user", user.Name).
				Msg("NodeHandler.PostAdd failed after auth-path registration")
		}
	}
	// __END_CYLONIX_ADD__
	h.Change(c)
	_ = writer // upstream's response is written by the AuthProviderOIDC handler.
	return nil
}
