...
func TestSQLiteMigrationAndDataValidation(t *testing.T) {
	// __CYLONIX_ADD__ this test diffs the post-migration schema against
	// upstream's SQL dumps, which do not include the cylonix fields
	// (namespace/network/login_name/capabilities/share-to relations/etc.).
	// The divergence is intentional — cylonix's schema has genuinely
	// different columns — so this test is expected to fail in the
	// cylonix fork. Skipped to keep the suite green.
	t.Skip("cylonix: schema intentionally diverges from upstream dumps")
	tests := []struct {
		dbPath   string
...
func TestConstraints(t *testing.T) {
	// __CYLONIX_ADD__ upstream's user-uniqueness model allows the same Name
	// with different provider_identifier values. Cylonix stores its tenant
	// UUID in User.Name and requires Name to be globally unique (gorm:"unique"),
	// which deliberately conflicts with the upstream test cases here. This
	// test is skipped in the cylonix fork; uniqueness is covered by
	// cylonix-specific tests.
	t.Skip("cylonix: User.Name is globally unique (UUID); upstream's duplicate-name-across-providers model does not apply")
	tests := []struct {
		name string
...
func TestSQLiteAllTestdataMigrations(t *testing.T) {
	// __CYLONIX_ADD__ iterates upstream .sql test dumps through the cylonix
	// migration chain. The post-migration schema includes cylonix fields
	// (namespace/network/login_name/capabilities/share-to relations) that
	// upstream's expected-state fixtures do not. Intentional divergence.
	t.Skip("cylonix: upstream test dumps don't include cylonix schema extensions")
	t.Parallel()
