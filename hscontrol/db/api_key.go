...
// GetAndValidateAPIKey validates an API key string (new or legacy format) and
// returns the API key record if valid. Used by the cylonix auth middleware to
// access scope metadata after validation.
func (hsdb *HSDatabase) GetAndValidateAPIKey(keyStr string) (*types.APIKey, bool, error) {
	...
	// Re-fetch with User preloaded so the auth middleware can read scope data.
	full, err := hsdb.GetAPIKey(key.Prefix)
	if err != nil {
