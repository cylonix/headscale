	...
	tests := []struct {
	...
	}{
		{
			...
			test: func(t *testing.T, db *HSDatabase) {
				...
				keyStr, apiKey, err := db.CreateAPIKey(nil, "", "", "", "", "") // __CYLONIX_MOD__
				require.NoError(t, err)
		...
		},
		{
			...
			test: func(t *testing.T, db *HSDatabase) {
				...
				keyStr, createdKey, err := db.CreateAPIKey(nil, "", "", "", "", "") // __CYLONIX_MOD__
				require.NoError(t, err)
		...
		},
		{
			...
			test: func(t *testing.T, db *HSDatabase) {
				...
				keyStr, _, err := db.CreateAPIKey(nil, "", "", "", "", "") // __CYLONIX_MOD__
				require.NoError(t, err)
		...
		},
		{
			...
			test: func(t *testing.T, db *HSDatabase) {
				...
				keyStr, _, err := db.CreateAPIKey(&expired, "", "", "", "", "") // __CYLONIX_MOD__
				require.NoError(t, err)
...
func TestGetAPIKeyByID(t *testing.T) {
	...
	// Create an API key
	_, apiKey, err := db.CreateAPIKey(nil, "", "", "", "", "") // __CYLONIX_MOD__
	require.NoError(t, err)
	require.NotNil(t, apiKey)
